...
if (isset($_POST['cari'])) {
    ...
    $q1 = mysqli_query($conn,'SELECT * FROM mahasiswa JOIN prodi ON mahasiswa.kode_prodi=prodi.kode_prodi');
    $w = mysqli_fetch_assoc($q1);
    
    $q = mysqli_query($conn,"SELECT * FROM mahasiswa where npm LIKE '%$c%' OR nama = '$c'");
...
?>
    ...
    mysqli_query ($conn, "UPDATE mahasiswa SET id='$id', nama='$nama', npm='$npm', email='$email', kode_prodi='$prog' where id='$id'");
    header("location:?admin");
    } 
    ...
    <table class="table table-striped">
        ...
        <form action="" method="post">
            ...
            <select name="ps" id="" class="form-control mt-3">
                <?php foreach (mysqli_query($conn,"SELECT * FROM prodi") as $rawr) : ?>
                <option value="<?= $rawr['kode_prodi'] ?>"><?= $rawr['nama_prodi'] ?></option>
                <?php endforeach ?>
            </select>
            <div class="">
        ...
        <?php elseif (isset($_POST['edit'])) : ?>
            ...
            <div class="form-group">
                <label class="form-label" for="w"></label>
                <input class="form-control" type="text" name="name" id="w" value="<?= $sw['nama'] ?>" autofocus="off" autocomplete="off">
            </div>
            <div class="form-group">
                <label class="form-label" for="d"></label>
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="text" name="ps" id="s" value="<?= $sw['kode_prodi'] ?>">
            </div>
            <div class="mt-3">
        ...
        <tbody>
        <?php 
        $q = mysqli_query($conn,"SELECT * FROM mahasiswa JOIN prodi ON mahasiswa.kode_prodi = prodi.kode_prodi");
